...
import { createTokenMetadata } from '@/services/metaplex';
...
export async function createCoin(params: CoinCreationParams, walletAddress: string, walletProvider: string): Promise<{
  ...
  try {
    ...
    if (params.includeMetadata) {
      toast.info("Starting metadata process...", {
        description: "This will involve several steps including image processing and on-chain updates",
        duration: 5000,
      });
      ...
      try {
        ...
        if (metadataResult.success) {
          ...
          console.error("Metadata creation failed with error:", metadataResult.error);
        ...
        console.error("Exception during metadata process:", metadataError);
